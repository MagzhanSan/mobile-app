...
export const shipmentsApi = {
  getShipments: async (status?: string): Promise<{ shipments: Shipment[] }> => {
    ...
    try {
      ...
      if (netInfo.isConnected && netInfo.isInternetReachable) {
        ...
        // Дополнительно кэшируем только in_transit рейсы
        if (status !== 'in_transit') {
          await shipmentsApi.cacheInTransitShipments();
        }
        ...
        if (cached) {
          // Если есть кэш и фильтр совпадает, возвращаем его
          if (cached.filterStatus === (status || '')) {
  ...
  updateShipment: async (
    ...
    return httpClient.put<Shipment>(`/shipments/${id}`, data);
  },
  ...
  cacheInTransitShipments: async (): Promise<void> => {
    try {
      const result = await httpClient.get<{ shipments: Shipment[] }>('/shipments?status=in_transit');
